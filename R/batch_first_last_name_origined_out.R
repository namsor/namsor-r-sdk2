  ...
  public = list(
    ...
    fromJSON = function(BatchFirstLastNameOriginedOutJson) {
      ...
      if (!is.null(BatchFirstLastNameOriginedOutObject$`personalNames`)) {
        self$`personalNames` <- sapply(BatchFirstLastNameOriginedOutObject$`personalNames`, function(x) {
    ...
    fromJSONString = function(BatchFirstLastNameOriginedOutJson) {
      BatchFirstLastNameOriginedOutObject <- jsonlite::fromJSON(BatchFirstLastNameOriginedOutJson)
      data.frame <- BatchFirstLastNameOriginedOutObject$`personalNames`
      self$`personalNames` <- vector("list", length = nrow(data.frame))
      for (row in 1:nrow(data.frame)) {
          personalNames.node <- FirstLastNameOriginedOut$new()
