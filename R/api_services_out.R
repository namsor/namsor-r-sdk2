# NamSor API v2
#
# NamSor API v2 : enpoints to process personal names (gender, cultural origin or ethnicity) in all alphabets or languages. Use GET methods for small tests, but prefer POST methods for higher throughput (batch processing of up to 100 names at a time). Need something you can't find here? We have many more features coming soon. Let us know, we'll do our best to add it! 
#
# The version of the OpenAPI document: 2.0.10
# Contact: contact@namsor.com
# Generated by: https://openapi-generator.tech

#' @docType class
#' @title APIServicesOut
#' @description APIServicesOut Class
#' @format An \code{R6Class} generator object
#' @field apiServices  list( \link{APIServiceOut} ) [optional]
#'
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
APIServicesOut <- R6::R6Class(
  'APIServicesOut',
  public = list(
    `apiServices` = NULL,
    initialize = function(`apiServices`=NULL, ...){
      local.optional.var <- list(...)
      if (!is.null(`apiServices`)) {
        stopifnot(is.vector(`apiServices`), length(`apiServices`) != 0)
        sapply(`apiServices`, function(x) stopifnot(R6::is.R6(x)))
        self$`apiServices` <- `apiServices`
      }
    },
    toJSON = function() {
      APIServicesOutObject <- list()
      if (!is.null(self$`apiServices`)) {
        APIServicesOutObject[['apiServices']] <-
          lapply(self$`apiServices`, function(x) x$toJSON())
      }

      APIServicesOutObject
    },
    fromJSON = function(APIServicesOutJson) {
      APIServicesOutObject <- jsonlite::fromJSON(APIServicesOutJson)
      if (!is.null(APIServicesOutObject$`apiServices`)) {
        self$`apiServices` <- ApiClient$new()$deserializeObj(APIServicesOutObject$`apiServices`, "array[APIServiceOut]", loadNamespace("namsor"))
      }
    },
    toJSONString = function() {
      jsoncontent <- c(
        if (!is.null(self$`apiServices`)) {
        sprintf(
        '"apiServices":
        [%s]
',
        paste(sapply(self$`apiServices`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )}
      )
      jsoncontent <- paste(jsoncontent, collapse = ",")
      paste('{', jsoncontent, '}', sep = "")
    },
    fromJSONString = function(APIServicesOutJson) {
      APIServicesOutObject <- jsonlite::fromJSON(APIServicesOutJson)
      self$`apiServices` <- ApiClient$new()$deserializeObj(APIServicesOutObject$`apiServices`, "array[APIServiceOut]", loadNamespace("namsor"))
      self
    }
  )
)
