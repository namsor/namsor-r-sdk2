...
#'
#'
#' country_batch [USES 10 UNITS] Infer the likely country of residence of up to 100 personal full names, or surnames. Assumes names as they are in the country of residence OR the country of origin.
#'
#'
...
#'
#'
#' diaspora_batch [USES 20 UNITS] Infer the likely ethnicity/diaspora of up to 100 personal names, given a country of residence ISO2 code (ex. US, CA, AU, NZ etc.)
#'
#'
...
#'
#'
#' origin_batch [USES 10 UNITS] Infer the likely country of origin of up to 100 names, detecting automatically the cultural context.
#'
#'
...
#'
#'
#' parsed_gender_batch Infer the likely gender of up to 100 fully parsed names, detecting automatically the cultural context.
#'
#'
#' parsed_gender_geo_batch Infer the likely gender of up to 100 fully parsed names, detecting automatically the cultural context.
#'
#'
#' us_race_ethnicity [USES 10 UNITS] Infer a US resident&#39;s likely race/ethnicity according to US Census taxonomy W_NL (white, non latino), HL (hispano latino),  A (asian, non latino), B_NL (black, non latino).
#'
#'
#' us_race_ethnicity_batch [USES 10 UNITS] Infer up-to 100 US resident&#39;s likely race/ethnicity according to US Census taxonomy.
#'
#'
#' us_race_ethnicity_zip5 [USES 10 UNITS] Infer a US resident&#39;s likely race/ethnicity according to US Census taxonomy, using (optional) ZIP5 code info. Output is W_NL (white, non latino), HL (hispano latino),  A (asian, non latino), B_NL (black, non latino).
#'
#'
#' us_zip_race_ethnicity_batch [USES 10 UNITS] Infer up-to 100 US resident&#39;s likely race/ethnicity according to US Census taxonomy, with (optional) ZIP code.
#'
#' }
#'
#' @export
PersonalApi <- R6::R6Class(
  ...
  public = list(
    ...
    parse_name = function(name_full, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()

      urlPath <- "/api2/json/parseName/{nameFull}"
      if (!missing(`name_full`)) {
        urlPath <- gsub(paste0("\\{", "nameFull", "\\}"), `name_full`, urlPath)
      }
    ...
    parse_name_batch = function(batch_personal_name_in, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()
    ...
    parse_name_geo = function(name_full, country_iso2, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()
    ...
    parse_name_geo_batch = function(batch_personal_name_geo_in, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()
