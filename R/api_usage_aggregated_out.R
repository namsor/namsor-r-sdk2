...
#'
#' @field data   \link[namsor:array]{   list(array[integer])   }  [optional]
#'
  ...
  public = list(
    ...
    fromJSON = function(APIUsageAggregatedOutJson) {
      ...
      if (!is.null(APIUsageAggregatedOutObject$`data`)) {
        self$`data` <- ApiClient$new()$deserializeObj(APIUsageAggregatedOutObject$`data`, "array[array[integer]]", "package:namsor")
      }
      if (!is.null(APIUsageAggregatedOutObject$`colHeaders`)) {
        self$`colHeaders` <- ApiClient$new()$deserializeObj(APIUsageAggregatedOutObject$`colHeaders`, "array[character]", "package:namsor")
      }
      if (!is.null(APIUsageAggregatedOutObject$`rowHeaders`)) {
        self$`rowHeaders` <- ApiClient$new()$deserializeObj(APIUsageAggregatedOutObject$`rowHeaders`, "array[character]", "package:namsor")
      }
    },
    toJSONString = function() {
      jsoncontent <- c(
        ...
        [%s]
',
        paste(unlist(lapply(self$`data`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA))), collapse=",")
        )},
        if (!is.null(self$`colHeaders`)) {
        sprintf(
        '"colHeaders":
